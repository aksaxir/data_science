...
sql_str = """
    ...
    SELECT *  INTO movies_top FROM (
        SELECT 
            movieId, COUNT(rating) as ratings_num, AVG(rating) as ratings_avg
        FROM ratings GROUP BY movieId HAVING AVG(rating) > 3 ORDER BY ratings_avg
    ) as _
"""
...
top_rated_content_ids = [
    i[0] for i in top_rated_query.filter(MoviesTop.ratings_num > 15).filter(MoviesTop.ratings_avg > 3.5).order_by(MoviesTop.ratings_avg).values(MoviesTop.movieid)
]

logger.info("Выборка из top_rated_content_ids\n{}".format([i for i in top_rated_content_ids[:5]]))
